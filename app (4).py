...
.cafe-header {
    ...
    background-image: radial-gradient(#E4CBA0 1px, transparent 1px);
    background-size: 22px 22px;
    border-radius: 18px;
    padding: 28px 32px 24px 32px;
    margin-bottom: 28px;
}
.cafe-header-top { display: flex; align-items: center; gap: 12px; margin-bottom: 14px; }
.cafe-icon-circle {
    width: 46px; height: 46px; border-radius: 50%;
    background: #EAD8B8; display: flex; align-items: center; justify-content: center;
    font-size: 20px;
}
.cafe-badge {
    display: inline-flex; align-items: center; gap: 6px;
    background: rgba(255,255,255,0.55); border-radius: 20px;
    padding: 5px 14px; font-size: 13px; font-weight: 700; color: #6B6647;
    margin-left: 4px;
}
.cafe-badge-dot { width: 10px; height: 10px; border-radius: 50%; background: #8B6F8E; display:inline-block; }
.cafe-title { font-size: 34px; font-weight: 900; color: #5A3A22; margin: 0; }
...
div[data-testid="stMetric"] {
    ...
    border-radius: 14px;
    padding: 14px 18px;
    box-shadow: 0 2px 8px rgba(90,58,34,0.06);
}
div[data-testid="stMetricLabel"] { color: #8A6F52; }
div[data-testid="stMetricValue"] { color: #5A3A22; }
...
.stTabs [aria-selected="true"] {
    background-color: #D9A45C !important;
    color: #2E1D12 !important;
}

div[data-testid="stDataFrame"] { border-radius: 12px; overflow: hidden; border: 1px solid #4A2F1E; }

.stButton > button, .stFormSubmitButton > button {
    background-color: #D9A45C; color: #2E1D12; border-radius: 10px; border: none;
    font-weight: 700;
}
.stButton > button:hover, .stFormSubmitButton > button:hover { background-color: #E8BB7C; color: #2E1D12; }

div[data-testid="stExpander"] { border: 1px solid #4A2F1E; border-radius: 12px; }
</style>
""", unsafe_allow_html=True)

st.markdown("""
<div class="cafe-header">
    <div class="cafe-header-top">
        <div class="cafe-icon-circle">☕</div>
        <div class="cafe-icon-circle" style="font-size:22px;">🐶</div>
        <span class="cafe-badge"><span class="cafe-badge-dot"></span>Cafe Ops 내부 도구</span>
    </div>
    <p class="cafe-title">인마이험블 재고관리</p>
</div>
""", unsafe_allow_html=True)
...
tab_dash, tab_check, tab_stock, tab_recipe, tab_report = st.tabs(
    ["📊 대시보드", "✅ 재고 체크 입력", "📦 재고 전체보기", "☕ 음료 레시피", "🧾 발주 리포트 (사장님용)"]
)
...
with tab_dash:
    ...
    for part in ["공통", "파트1", "파트2", "파트3"]:
        ...
        with st.expander(f"{part} ({PART_OWNERS.get(part, part)}) — 재료 {len(part_df)}개"):
            chart_df = part_df[["name", "count"]].sort_values("count", ascending=True)
            bar_chart = (
                alt.Chart(chart_df)
                .mark_bar(cornerRadiusTopRight=6, cornerRadiusBottomRight=6, color="#8B5E34")
                .encode(
                    x=alt.X("count:Q", title="재고(개수)"),
                    y=alt.Y("name:N", sort="-x", title=None),
                    tooltip=[alt.Tooltip("name:N", title="재료"), alt.Tooltip("count:Q", title="재고")],
                )
                .properties(height=28 * len(chart_df) + 20)
            )
...
with tab_check:
        ...
        with st.form("stock_check_form"):
            ...
            for row in my_ings:
                ...
                inputs[row["id"]] = st.number_input(
                    f'{row["name"]} ({row["package_label"]} 단위)',
                    min_value=0,
    ...
    with get_conn() as conn:
        recent = pd.read_sql_query(
            """
            SELECT i.name AS 재료명, s.quantity AS 체크당시수량,
                   i.package_size AS 포장용량, i.package_label AS 포장단위,
                   s.checked_by AS 체크한사람, s.checked_at AS 체크시각
    ...
    if not recent.empty:
        recent["재고"] = (recent["체크당시수량"] / recent["포장용량"]).round(1).astype(str) + recent["포장단위"]
        recent = recent[["재료명", "재고", "체크한사람", "체크시각"]]
...
with tab_stock:
    ...
    with st.form("add_ingredient_form", clear_on_submit=True):
        ...
        part_sel = c2.selectbox("파트", ["공통", "파트1", "파트2", "파트3"])
        c3, c4, c5 = st.columns(3)
        package_label = c3.text_input("포장 단위 이름 (병/봉/통/개)", value="병")
        package_size = c4.number_input("1개(병/봉/통)당 용량", min_value=1.0, step=1.0, value=500.0)
        start_count = c5.number_input("현재 개수", min_value=0, step=1, value=1)
        if st.form_submit_button("등록") and name.strip():
            with get_conn() as conn:
                ...
                conn.execute(
                    ...
                    VALUES (?,?,?,?,?,?,?,?)""",
                    (
                        name.strip(), "", start_count * package_size, package_size,
...
with tab_recipe:
    ...
    st.divider()
    st.subheader("음료 레시피 구성")
    with get_conn() as conn:
        menus = conn.execute("SELECT id, name, part FROM menus ORDER BY part, name").fetchall()
        ingredients = conn.execute("SELECT id, name FROM ingredients ORDER BY part, name").fetchall()

    if not menus:
        st.info("먼저 음료 메뉴를 등록해주세요.")
    else:
        menu_options = {f'{row["name"]} ({row["part"]})': row["id"] for row in menus}
        selected_menu_label = st.selectbox("음료 선택", menu_options.keys())
        selected_menu_id = menu_options[selected_menu_label]

        with get_conn() as conn:
            current_recipe = pd.read_sql_query(
                """
                SELECT i.name AS 재료명, r.amount_per_serving AS 소모량
        ...
        st.write(f"**'{selected_menu_label}' 음료 레시피 (1잔 기준)**")
        st.dataframe(current_recipe, use_container_width=True, hide_index=True)

        if ingredients:
            ing_options = {row["name"]: row["id"] for row in ingredients}
...
with tab_report:
    ...
    if checks.empty:
        ...
        if est_df.empty or est_df["추정 판매잔수"].sum() == 0:
            ...
            menu_chart = (
                alt.Chart(est_df)
                .mark_bar(cornerRadiusTopRight=6, cornerRadiusBottomRight=6, color="#5A3A22")
                .encode(
                    x=alt.X("추정 판매잔수:Q"),
                    y=alt.Y("메뉴:N", sort="-x", title=None),
                    tooltip=["메뉴", "추정 판매잔수"],
                )
                .properties(height=32 * len(est_df) + 20)
            )
            st.altair_chart(menu_chart, use_container_width=True)
